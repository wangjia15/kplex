...
/**
 * Resolves lens values without expanding GraphPage or the persisted graph snapshot.
 * Arbitrary note properties are read lazily from Obsidian's MetadataCache only when a predicate
 * references `note.<property>`. K-Plex-native graph fields continue to come from GraphPage.
 */
export class GraphPredicateEngine {
  ...
  private resolve(value: GraphPredicateValue, context: GraphPredicateContext): unknown {
    ...
    if (namespace === "file") return this.resolveFile(context.node.page, key);
    if (namespace === "this") return context.center ? this.resolvePage(context.center, key) : undefined;
    return this.resolveNoteProperty(context.node.page, key);
  }

  private resolveNode(node: GraphPredicateNodeContext, key: string): unknown {
    if (key === "label") return node.label ?? node.page.name;
    return this.resolvePage(node.page, key);
  }

  private resolvePage(page: GraphPage, key: string): unknown {
    ...
    if (key === "isFolder") return page.isFolder;
    if (key === "isTag") return page.isTag;
  ...
  private resolveEvidence(evidence: RelationEvidence | undefined, key: string): unknown {
    if (!evidence) return undefined;
    return lookupRecordValue(unknownRecord(evidence), key);
  }

  private resolveFile(page: GraphPage, key: string): unknown {
    ...
    if (key === "mtime") return file.stat.mtime;
    if (key === "ctime") return file.stat.ctime;
    if (key === "size") return file.stat.size;
    if (key === "tags") return page.tags;
    return undefined;
  }

  private resolveNoteProperty(page: GraphPage, key: string): unknown {
    if (!page.file || page.file.extension !== "md") return undefined;
    const frontmatter = unknownRecord(this.app.metadataCache.getFileCache(page.file)?.frontmatter);
    return lookupRecordValue(frontmatter, key);
  }
}
